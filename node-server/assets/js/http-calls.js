    ...
    .then((data) => {
      ...
      if (data.data.inference === "Inconclusive") {
        showPredictionResult(
          `We couldn't relaibly determine if the above image is infected or not, confidence score is ${data.data.confidenceScore}`
        );
        return;
      }

      showPredictionResult(
        `The above image has a '${chancePercent.toFixed(2)}%' chance of been '${
          data.data.inference
        }'`
      );
    })
